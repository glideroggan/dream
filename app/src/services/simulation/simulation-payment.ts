...
//         }
//     }
//     // first look if we find the transaction reference as a paid transaction
//     // if we find it, then we update the loan entity with the updated data
//     const transactionRepo = repositoryService.getTransactionRepository();
//     const transaction = await transactionRepo.getByReference(lastTransactionReference);
//     if (transaction) {
//         // update the loan with the new data
//         loan.metadata.currentlyOwed -= transaction.amount;
//         loan.paymentsMade += 1;
//         loan.paymentsRemaining -= 1;
//         loan.remainingAmount -= transaction.amount;
//         await loanRepo.update(task.productId, loan);
//     }
